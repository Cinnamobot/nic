...
**AI（LLM）に「新潟の今」を教えられるデータ基盤。**

昨今、旅行計画を AI で立てる人が増えています。「新潟に何がある？」「雪はどのくらい？」「雨の日はどこへ？」——
しかし ChatGPT や Claude に聞いても、AI は新潟の最新データを持っていないため、正確には答えられません。

NIC（Niigata Information Connector）は、**AI が新潟の情報を得られやすい状態を作る**ための基盤です。
新潟特化のモデルを作るのではなく（莫大なコストがかかる・特定の LLM に縛られる）、
**新潟特化のツールを提供**することで、どんな AI でも・今すぐ・安価に新潟の実データを扱えるようにします。

インストール 1 コマンド + 設定 1 箇所で、あなたの AI アシスタントが**新潟県の実データ（気象・防災・観光・統計）を参照して、正確に答えてくれるようになります**。
