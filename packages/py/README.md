# NIC — Niigata Information Connector (Python)

Python 版 NIC。新潟県の情報（気象・統計・観光・防災・オープンデータ）にアクセスする
**SDK（ライブラリ）+ CLI（`ngt`）+ MCP（`ngt-mcp`）**。
システム全体のコンセプト・ストーリーはリポジトリルートの [README](../README.md) を参照。
...
// 2) claude_desktop_config.json にこれを追加するだけ
{
  "mcpServers": {
    "nic": {
      "command": "uv",
      "args": ["--directory", "C:/path/to/nic/packages/py", "run", "ngt-mcp"]
    }
  }
}
```
